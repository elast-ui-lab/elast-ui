...
import { ComboBoxProps, ComboBoxContextType, DataType, OptionProps } from './types';

const ComboBox = <T extends DataType>(props: ComboBoxProps<T>) => {
  const { id, className, value, children, required, ariaLabel, onValueChange, ...restProps } =
    props;
    ...
    if (optionWrapper?.props?.children) {
      const validOptions = React.Children.toArray(optionWrapper.props.children).filter(
        (child): child is ReactElement =>
          isValidElement(child) && (child.type as React.FunctionComponent)?.displayName === 'Option'
